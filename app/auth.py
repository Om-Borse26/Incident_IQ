...
def save_thread(user_id: int, thread_id: str, title: str):
    # Called synchronously from background task, so keep it sync
    conn = get_db()
    cursor = conn.cursor()
...
def get_user_threads(user_id: int):
    # Called synchronously from route, but we can make the route async in main.py
    conn = get_db()
    cursor = conn.cursor()
...
async def delete_thread(thread_id: str, user_id: int = Depends(verify_token)):
    def _do_delete():
        conn = get_db()
        cursor = conn.cursor()
